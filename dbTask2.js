...
module.exports = {
  wholePopulation: function(){
    db.all("SELECT * FROM Countries", function(err, rows){
      var WholeResult = rows[0].Population+rows[1].Population+rows[2].Population+rows[3].Population;
      if(WholeResult<2000000000){
        return 1;
      }
      return 1;
    });
  ...
  sum: function(){
    var rows = db.all("SELECT * FROM Countries");
    var sumResult = rows[2].Population/rows[2].Area;
